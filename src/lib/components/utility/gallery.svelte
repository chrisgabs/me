...
<Carousel.Root bind:api opts={apiOptions} class="group border border-gray-400 rounded-sm {galleryIsExpandable ? '' : 'cursor-default'}" >
    ...
    {#if galleryIsExpandable}
        <div class="absolute bottom-3 right-3 bg-white p-1 border border-gray-400 rounded-md opacity-15 group-hover:opacity-85 transition-all duration-300" >
...
<Dialog.Root bind:open={dialogOpened}>
    ...
    >
        ...
        <Carousel.Root bind:api opts={apiOptions} class="group mx-5 border rounded-sm border-gray-400" >
            ...
            {#if images.length !== 1}
                <div class="{dialogOpened ? "flex" : "hidden"} gap-2 absolute left-1 -bottom-12">
                    <button class="p-1 px-2 flex items-center justify-center bg-white rounded-lg opacity-70 border border-b-2 border-gray-400 hover:opacity-100" on:click={(e) => {
                        api.scrollPrev();
                        e.stopPropagation();
                    }}>
            ...
            <Dialog.Close class="absolute -top-12 right-1 flex items-center justify-center bg-white rounded-lg opacity-70 border border-b-2 border-gray-400 hover:opacity-100 px-2 py-1" >
